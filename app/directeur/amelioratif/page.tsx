...
type Ticket = {
  id: string; titre: string; priorite: string; statut: string; created_at: string;
...
export default function DirecteurAmelioratifPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('tickets')
      ...
      .eq('type_intervention', 'ameliorative').order('created_at', { ascending: false });
    ...
    <div style={{ padding: '16px', paddingBottom: 32 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map(t => (
            <Link key={t.id} href={`/directeur/tickets/${t.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
